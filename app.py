...
st.write("District: Farrukhabad | Year: 2026")

# Create Tabs so everything is clean
tab1, tab2 = st.tabs(["📸 AI Scanner", "💹 Mandi Rates"])

with tab1:
    st.header("Potato Health Scanner")
    # This opens the camera directly
    pic = st.camera_input("Take a photo of the potato")
    ...
    if pic:
        ...
        st.write("**Grade:** A (Quality)")
        st.write("**Current Mandi Price:** ₹510/Quintal")
        st.info("💡 Suggestion: Proper storage will maintain quality.")

with tab2:
    st.header("Farrukhabad Mandi Updates")
    st.table({
        "Variety": ["Local", "Badshah", "Pukhraj"],
        "Rate (₹/q)": ["510", "615", "540"],
        "Status": ["Stable", "High", "Stable"]
    })
import streamlit as st
import cv2
import numpy as np
